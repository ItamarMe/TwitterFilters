...
const deleteReposts = (element) => {
    element.querySelectorAll(`path[d=${repostSVG}]`)
        .forEach(el => {
            el.closest('[data-testid="cellInnerDiv"]').classList.add('hide-repost');
        });
}

const showReposts = (element) => {
    element.querySelectorAll(`.hide-repost`)
        .forEach(el => {
